...
#define HAIER_PRODUCT_CE81B			0x10f8 /* wive */
...
static const struct usb_device_id option_ids[] = {
	...
	{ USB_DEVICE(LONGCHEER_VENDOR_ID, FOUR_G_SYSTEMS_PRODUCT_W14),
  	...
  	{ USB_DEVICE(LONGCHEER_VENDOR_ID, SPEEDUP_PRODUCT_SU9800) },
	{ USB_DEVICE(LONGCHEER_VENDOR_ID, ZOOM_PRODUCT_4597) },
	{ USB_DEVICE(LONGCHEER_VENDOR_ID, IBALL_3_5G_CONNECT) },
	{ USB_DEVICE(HAIER_VENDOR_ID, HAIER_PRODUCT_CE100) },
	{ USB_DEVICE(HAIER_VENDOR_ID, HAIER_PRODUCT_CE81B) }, /* wive */
	/* Pirelli  */
...
static struct usb_driver option_generic_driver = {
	.name       = "option_generic",
	.probe      = usb_serial_probe,
	.disconnect = usb_serial_disconnect,
...
static struct usb_serial_driver option_generic = {
	.driver = {
		.owner =	THIS_MODULE,
		.name =		"option_generic1",
	},
	.description       = "GSM modem (1-port) generic",
	.usb_driver        = &option_generic_driver,
	.id_table          = generic_device_ids,
	.num_interrupt_in  = NUM_DONT_CARE,
	.num_bulk_in       = NUM_DONT_CARE,
	.num_bulk_out      = NUM_DONT_CARE,
...
static int __init option_init(void)
{
	...
	generic_device_ids[0].match_flags = USB_DEVICE_ID_MATCH_DEVICE;

	retval = usb_serial_register(&option_1port_device);
	if (retval)
		goto failed_1port_device_register;

	retval = usb_serial_register(&option_generic);
	if (retval)
		goto failed_1port_device_generic;

	retval = usb_register(&option_driver);
	if (retval)
		goto failed_driver_register;

	retval = usb_register(&option_generic_driver);
	if (retval)
		goto failed_generic_driver_register;

	info(DRIVER_DESC ": " DRIVER_VERSION);

	return 0;

failed_generic_driver_register:
	usb_deregister(&option_driver);
failed_driver_register:
	usb_serial_deregister (&option_generic);
failed_1port_device_generic:
...
{
	usb_deregister (&option_driver);
	usb_deregister (&option_generic_driver);
	usb_serial_deregister (&option_1port_device);
	usb_serial_deregister (&option_generic);
}
