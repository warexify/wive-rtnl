 ...
 *
 * Version:	$Id: route.c,v 1.1.1.1 2007-05-25 06:49:59 bruce Exp $
 *
 * Authors:	Ross Biro
...
{
	return HASH_3WORDS((__force u32)(__be32)(daddr),
		    ...
		    __be32 saddr, struct net_device *dev)
{
	...
	if (new_gw == old_gw || !IN_DEV_RX_REDIRECTS(in_dev)
	    || ipv4_is_multicast(new_gw) || ipv4_is_lbcast(new_gw) || ipv4_is_zeronet(new_gw))
...
static void ip_rt_update_pmtu(struct dst_entry *dst, u32 mtu)
{
	...
	if (dstmtu > mtu && mtu >= 68 &&
		...
		dst_set_expires(dst, max(1UL, jiffies + ip_rt_mtu_expires));
	}
}
