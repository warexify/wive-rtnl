 ...
 *
 * Version:	$Id: ip_fragment.c,v 1.1.1.1 2007-05-25 06:49:59 bruce Exp $
 *
...
static void ip_expire(unsigned long arg)
{
	...
	if ((qp->last_in&FIRST_IN) && qp->fragments != NULL) {
	    ...
	    rcu_read_lock();
	    head->dev = dev_get_by_index(qp->iif);
	    if (!head->dev)
		goto out_rcu_unlock;
	    ...
	    */
	    if (qp->user == IP_DEFRAG_CONNTRACK_IN && !head->dst) {
		 ...
		 */
		if (((struct rtable *)head->dst)->rt_type != RTN_LOCAL)
			goto out_rcu_unlock;
	    }
			 ...
			 struct net_device *dev)
{
	...
	iph->frag_off = qp->max_size ? htons(IP_DF) : 0;
