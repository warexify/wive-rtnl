...
retry:
	...
	tmp_prefered_lft = min_t(__u32,
				 ifp->prefered_lft,
				 idev->cnf.temp_prefered_lft  + age - desync_factor / HZ);
		...
		if (ifp == NULL && valid_lft) {
			...
			if (!max_addresses ||
				...
				ifp = ipv6_add_addr(in6_dev, &addr, pinfo->prefix_len,
						    ...
						    addr_flags, valid_lft, prefered_lft);
			  ...
			  __u8 ifa_flags, __u32 prefered_lft, __u32 valid_lft)
{
	...
	ifp = ipv6_add_addr(idev, pfx, plen, scope, ifa_flags, valid_lft, prefered_lft);
...
static void sit_add_v4_addrs(struct inet6_dev *idev)
{
	...
	if (addr.s6_addr32[3]) {
		ifp = ipv6_add_addr(idev, &addr, 128, scope, IFA_F_PERMANENT, 0, 0);
		if (!IS_ERR(ifp)) {
			spin_lock_bh(&ifp->lock);
			ifp->flags &= ~IFA_F_TENTATIVE;
			spin_unlock_bh(&ifp->lock);
...
static void init_loopback(struct net_device *dev)
{
	...
	ifp = ipv6_add_addr(idev, &in6addr_loopback, 128, IFA_HOST, IFA_F_PERMANENT, 0, 0);
	if (!IS_ERR(ifp)) {
		spin_lock_bh(&ifp->lock);
		ifp->flags &= ~IFA_F_TENTATIVE;
		spin_unlock_bh(&ifp->lock);
