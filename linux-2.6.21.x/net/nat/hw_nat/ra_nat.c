...
#if defined (CONFIG_RA_HW_NAT_PCI)
		i = DP_NIC0;
		DstPort[i] = ra_dev_get_by_name("usbeth0");	// USB0 interface name
		if (DstPort[i])
		    i = DP_NIC1;
		DstPort[i] = ra_dev_get_by_name("usbeth1");	// USB1 interface name
#endif
	} else {
...
//		SetAclFwd(1); // wdf?
	...
	printk("Ralink HW NAT %s Module Enabled, ASIC: %s, REV: %04X, FoE Size: %d\n",
		HW_NAT_MODULE_VER, chip_id, chip_rev_id, FOE_4TB_SIZ);
